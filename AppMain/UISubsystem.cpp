...
namespace MyWeb {
    namespace UI {
        ...
        void CUISubsystem::initialize(Poco::Util::Application& app)
        {
            ...
            m_hCatchDll = ::LoadLibraryW(A2W_UTF8(appDir + "StockCatcherDll.dll").c_str());

            typedef bool(*pInitStockCatcher)(const char* appDir);

            pInitStockCatcher pInit = (pInitStockCatcher)(GetProcAddress(m_hCatchDll, "InitStockCatcher"));
        ...
        std::string CUISubsystem::GetCaipiaoInfo(const std::string& strCmd)
        {
            typedef bool (*pGetCaiPiaoInfo)(const char* cmdJson, char* pRet, int nLen);

            pGetCaiPiaoInfo pFunc = (pGetCaiPiaoInfo)(GetProcAddress(m_hCatchDll, "GetCaiPiaoInfo"));

            char* pRet = new char[2048];
            std::string strRet;
            if (pFunc(strCmd.c_str(), pRet, 2048))
            {
                strRet = pRet;
            }
            else
