...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->SetInset(m_pOwner->GetInset());
        m_pLayout->SetBkColor(0xFFFFFFFF);
        if (m_pOwner->GetFocusBorderColor() != 0)
        {
            m_pLayout->SetBorderColor(m_pOwner->GetFocusBorderColor());
        }
        else
        {
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 ) {
        DWORD dwSelectedTextColor = GetListInfo()->dwSelectedTextColor;
        GetListInfo()->dwSelectedTextColor = GetListInfo()->dwTextColor;
        CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
        ...
        GetListInfo()->dwSelectedTextColor = dwSelectedTextColor;
    }
}
