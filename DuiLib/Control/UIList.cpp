...
bool CListUI::Add(CControlUI* pControl)
{
    ...
    // We also need to recognize header sub-items
    if( pControl->GetInterface(DUI_CTR_LISTHEADERITEM) != NULL ) {
...
void CListUI::SetPos(RECT rc)
{
    ...
    //int iPosX = rcHeader.left;
    //SIZE szAvailable = { rcHeader.right - rcHeader.left, rcHeader.bottom - rcHeader.top };
    //iPosX -= m_pList->GetScrollPos().cx;
    for (int i = 0; i < m_ListInfo.nColumns; i++) {
        CControlUI* pControl = static_cast<CControlUI*>(m_pHeader->GetItemAt(i));
        if (!pControl->IsVisible()) continue;
        if (pControl->IsFloat()) continue;

       /* SIZE szCtrl = pControl->EstimateSize(szAvailable);
        RECT rcCtrl = { iPosX, rcHeader.top, iPosX + szCtrl.cx, rcHeader.top + szCtrl.cy };
        pControl->SetPos(rcCtrl);*/
        const RECT& rcPos = pControl->GetPos();
        m_ListInfo.rcColumn[i] = rcPos;
        //iPosX = rcPos.right;
    }
...
void CListUI::SetSepItemImage(LPCTSTR pStrSepItemImage)
{
    m_ListInfo.sSepItemImage = pStrSepItemImage;
    Invalidate();
}

void CListUI::SetSepItemWidth(int nSepItemWidth)
{
    m_ListInfo.nSepItemWidth = nSepItemWidth;
    Invalidate();
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("headerbkcolor")) == 0 ) 
    {
    ...
    else if( _tcscmp(pstrName, _T("itemsepimage")) == 0 ) SetSepItemImage(pstrValue);
    else if( _tcscmp(pstrName, _T("itemsepwidth")) == 0 ) SetSepItemWidth(_ttoi(pstrValue));
...
void CListHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcscmp(pstrName, _T("dragable")) == 0 ) SetDragable(_tcscmp(pstrValue, _T("true")) == 0);
        ...
        std::wstring strVal(pstrValue);
        size_t pos = strVal.find(L"top");
        if (pos != std::wstring::npos)
        {
            m_uTextStyle &= ~(DT_BOTTOM | DT_VCENTER);
            m_uTextStyle |= (DT_TOP | DT_SINGLELINE);
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 3, strVal.size()));
        }
        else if( (pos = strVal.find(L"vcenter")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_TOP | DT_BOTTOM );            
            m_uTextStyle |= (DT_VCENTER | DT_SINGLELINE);
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 7, strVal.size()));
        }
        else if( (pos = strVal.find(L"bottom")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_TOP | DT_VCENTER);
            m_uTextStyle |= (DT_BOTTOM | DT_SINGLELINE);
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 6, strVal.size()));
        }

        if( (pos = strVal.find(L"center")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_LEFT | DT_RIGHT );
            m_uTextStyle |= DT_CENTER;
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 6, strVal.size()));
        }
        else if( (pos = strVal.find(L"right")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_LEFT | DT_CENTER);
            m_uTextStyle |= DT_RIGHT;
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 5, strVal.size()));
        }
        else if( (pos = strVal.find(L"left")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_CENTER | DT_RIGHT);
            m_uTextStyle |= DT_LEFT;
            strVal = strVal.substr(0, pos).append(strVal.substr(pos + 4, strVal.size()));
        }
        if( (pos = strVal.find(L"wrap")) != std::wstring::npos) {
            m_uTextStyle &= ~(DT_SINGLELINE | DT_TOP | DT_BOTTOM | DT_VCENTER);
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
        ...
        NULL, NULL, nLinks, DT_SINGLELINE | DT_VCENTER | pInfo->uTextStyle);
    else
        CRenderEngine::DrawText(hDC, m_pManager, rcText, m_sText, iTextColor, \
        pInfo->nFont, DT_SINGLELINE | DT_VCENTER | pInfo->uTextStyle);
}
...
void CListTextElementUI::SetItemText(LPCTSTR pstrText)
{
    if( m_pOwner == NULL ) return;
    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    if( m_aTexts.GetSize() > pInfo->nColumns ) return;
    int iIndex = m_aTexts.GetSize();
    m_aTexts.Add(NULL);
    m_aTexts.SetAt(iIndex, new CDuiString(pstrText));
}

int CListTextElementUI::GetItemTextCount() const
{
...
void CListTextElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        if ( pInfo->nSepItemWidth != 0 ) {
            RECT rcThumb = GetItemThumbRect(rcItem, pInfo->nSepItemWidth);
            rcThumb.left -= m_rcItem.left;
            rcThumb.top -= m_rcItem.top;
            rcThumb.right -= m_rcItem.left;
            rcThumb.bottom -= m_rcItem.top;

            CRenderEngine::DrawRect(hDC, rcThumb, pInfo->nSepItemWidth, GetAdjustColor(pInfo->dwLineColor));
        }
    }
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        {
            RECT rcLine = { pInfo->rcColumn[i].right, m_rcItem.top, pInfo->rcColumn[i].right, m_rcItem.bottom };
            CRenderEngine::DrawLine(hDC, rcLine, 1, 0xc0c0c0);
        }
...
{
    CDuiString strName(pstrName);
    if (strName.GetLength() >= 8)
    {
        if (strName.Left(8) == _T("itemtext"))
        {
            SetItemText(pstrValue);
            return;
        }
    }
...
RECT CListTextElementUI::GetItemThumbRect(const RECT& rc, int nSepItemWidth) const
{
    if( nSepItemWidth >= 0 ) return CDuiRect(rc.right - nSepItemWidth, rc.top, rc.right, rc.bottom);
    else return CDuiRect(rc.left, rc.top, rc.left - nSepItemWidth, rc.bottom);
}
