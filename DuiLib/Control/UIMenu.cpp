...
void CMenuWnd::Init(CMenuElementUI* pOwner, const CDuiString& strXML, POINT point,
...
LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    if( m_pOwner != NULL) {
        ...
        CControlUI* pRoot = builder.Create(m_strXML.GetData(), UINT(0), this, &m_pm);
        ShowWindow(true);
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
        if( IsEnabled() ){
            ...
            else
            {
                ...
                if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
                {
                    TNotifyUI msg = {DUI_MSGTYPE_MENUITEMECLICK};
                    msg.wParam = (WPARAM)(new CDuiString(GetName()));
                    msg.lParam = (GetChecked() ? TRUE : FALSE);
