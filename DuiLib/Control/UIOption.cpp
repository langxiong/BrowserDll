...
Label_ForeImage:
    ...
    int COptionUI::GetGroupIndex() const
    {
        if (!m_pManager)
        {
            return -1;
        }
        auto strGroupName = GetGroup();
        auto pOptGroup = m_pManager->GetOptionGroup(strGroupName);
        for (int i = 0; i < pOptGroup->GetSize(); i++) {
            COptionUI* pControl = static_cast<COptionUI*>(pOptGroup->GetAt(i));
            if (pControl == this) {
                return i;
            }
        }
