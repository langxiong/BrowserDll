...
namespace DuiLib
{
    ...
    long CEditUI::GetCurSel() const
    {
        if (!m_pWindow)
        {
            return -1;
        }
        return Edit_GetSel(*m_pWindow);
    }
    ...
    void CEditUI::PaintText(HDC hDC)
    {
        ...
        if (m_pManager)
        {
            LONG lEditHeight = m_pManager->GetFontInfo(GetFont())->tm.tmHeight;
            if (lEditHeight < (rc.bottom - rc.top))
            {
                rc.top += (rc.bottom - rc.top - lEditHeight) / 2;
                rc.bottom = rc.top + lEditHeight;
            }
        }
