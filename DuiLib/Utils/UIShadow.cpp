...
void CShadowUI::OnFinalMessage(HWND hWnd)
{
    for (int i = 0; i < sm_shadowWnds.GetSize(); ++i)
    {
        CShadowUI* pShadowUI = reinterpret_cast<CShadowUI*>(sm_shadowWnds.GetAt(i));
        if (pShadowUI->m_hHostWnd == hWnd)
        {
            SetWindowLongPtr(hWnd, GWL_WNDPROC, pShadowUI->m_OriParentProc);
            sm_shadowWnds.Remove(i);
            break;
        }
    }
}

CShadowUI* CShadowUI::GetShadowUI(HWND hWnd)
{
    for (int i = 0; i < sm_shadowWnds.GetSize(); ++i)
    {
        CShadowUI* pShadowUI = reinterpret_cast<CShadowUI*>(sm_shadowWnds.GetAt(i));
...
void CShadowUI::Create(CPaintManagerUI* pPaintManager)
{
    ...
    m_hWnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT, strWndClassName, NULL,
        ...
        CW_USEDEFAULT, 0, 0, 0, m_hHostWnd, NULL, CPaintManagerUI::GetInstance(), NULL);
    
    if (!m_hWnd)
        return;

    sm_shadowWnds.Add(this);
    ...
    m_OriParentProc = GetWindowLongPtr(m_hHostWnd, GWLP_WNDPROC);

#pragma warning(disable: 4311)    // temporrarily disable the type_cast warning in Win32
    SetWindowLongPtr(m_hHostWnd, GWLP_WNDPROC, (LONG_PTR)ParentProc);
...
{
    CShadowUI *pThis = GetShadowUI(hwnd);
    _ASSERT(pThis);
...
void CShadowUI::Update(HWND hParent)
{
    ...
    ::UpdateLayeredWindow(m_hWnd, NULL, &ptDst, &WndSize, hMemDC,
...
int CShadowUI::GetSize() const
{
    return static_cast<int>(m_nSize);
}

unsigned int CShadowUI::GetSharpness() const
{
    return static_cast<unsigned int>(m_nSharpness);
}

unsigned int CShadowUI::GetDarkness() const
{
    return static_cast<unsigned int>(m_nDarkness);
}

POINT CShadowUI::GetPoisition() const
{
    POINT pt = { m_nxOffset, m_nyOffset };
    return pt;
}

COLORREF CShadowUI::GetColor() const
{
    return m_Color;
}

CDuiString CShadowUI::GetImage() const
{
    return m_sShadowImage;
}
...
DuiLib::CStdPtrArray CShadowUI::sm_shadowWnds;
