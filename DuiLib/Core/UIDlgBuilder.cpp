            ...
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    if( _tcscmp(pstrName, _T("size")) == 0 ) {
                        ...
                        auto size = pManager->GetInitSize();
                        // 防止childwindow重设主窗口的initsize
                        if (size.cx == 400 && size.cy == 400)
                        {
                            LPTSTR pstr = NULL;
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        //树控件XML解析
            ...
            case 11:
                ...
                break;
            case 12:
                if (_tcscmp(pstrClass, DUI_CTR_CAMERALAYOUT) == 0)            pControl = new CCameraLayoutUI;
                break;
            case 13:
                if (_tcscmp(pstrClass, DUI_CTR_NEWWEBBROWSER) == 0)              pControl = new CNewWebBrowserUI;
                break;
            case 14:
