...
void CPaintManagerUI::Init(HWND hWnd, bool isFilterMsg)
{
...
bool CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& /*lRes*/)
{
    ...
    {
        bool bHandled = false;
        static_cast<IMessageFilterUI*>(m_aPreMessageFilters[i])->MessageHandler(uMsg, wParam, lParam, bHandled);
        if( bHandled ) {
            return true;
        }
    }
    ...
    case WM_KEYDOWN:
        {
           ...
           if (wParam == VK_TAB) 
           {
                ...
                {
                    if (m_pFocus->GetInterface(DUI_CTR_RICHEDIT))
                    {
...
//#endif
    ...
    switch( uMsg ) {
    case WM_USER_DUILIB_DELAY_CLEAN_UP:
    {
    ...
    case WM_USER_DUILIB_DELAY_DESTROY_WNDS:
    {
        for (int i = 0; i < m_aDelayedDestroyedWnds.GetSize(); ++i)
        {
            delete static_cast<CWindowWnd*>(m_aDelayedCleanup[i]);
        }
        m_aDelayedDestroyedWnds.Empty();
        break;
    }
    case WM_USER_DUILIB_DELAY_DESTROY_THREADS:
    {
        for (int i = 0; i < m_aDelayedDestroyedThreads.GetSize(); ++i)
        {
            std::thread* pThread = static_cast<std::thread*>(m_aDelayedDestroyedThreads[i]);
            pThread->join();
            delete pThread;
        }
        m_aDelayedDestroyedThreads.Empty();
        break;
    }
    case WM_CLOSE:
        {
   ...
   case WM_PAINT:
    ...
    case WM_CTLCOLORSTATIC:
        ...
        break;
    case WM_USER_DUILIB_ASYNC_NOTIFY:
        {
            TNotifyUI* pMsg = NULL;
    ...
    case WM_USER_UPDATE_UI_TEXT_ASYNC:
        {
            CDuiString* pStrText = reinterpret_cast<CDuiString*>(lParam);
            if (!pStrText)
            {
                return false;
            }

            CControlUI* pCtrl = reinterpret_cast<CControlUI*>(wParam);
            if (pCtrl)
            {
                pCtrl->SetText(pStrText->GetData());
            }
            delete pStrText;
            return true;
        }
    case WM_USER_UPDATE_UI_PROGRESS_VAL_ASYNC:
        {
            CControlUI* pCtrl = reinterpret_cast<CControlUI*>(wParam);
            if (pCtrl)
            {
                CProgressUI* pProgress = static_cast<CProgressUI*>(pCtrl->GetInterface(DUI_CTR_PROGRESS));
                if (pProgress)
                {
                    pProgress->SetValue(static_cast<int>(lParam));
                }
            }
            return true;
        }
    case WM_USER_UPDATE_UI_OPTION_STATUS_ASYNC:
        {
            CControlUI* pCtrl = reinterpret_cast<CControlUI*>(wParam);
            if (pCtrl)
            {
                COptionUI* pOpt = static_cast<COptionUI*>(pCtrl->GetInterface(DUI_CTR_OPTION));
                if (pOpt)
                {
                    pOpt->Selected(lParam ? true : false);
                }
            }
            return true;
        }
...
void CPaintManagerUI::AddDelayedCleanup(CControlUI* pControl)
{
    ...
    ::PostMessage(m_hWndPaint, WM_USER_DUILIB_DELAY_CLEAN_UP, 0, 0L);
}
...
bool CPaintManagerUI::UpdateUITextAsync(const CDuiString& strText, CControlUI* pCtrl)
{
    if (!pCtrl || !::IsWindow(GetPaintWindow()))
    {
        return false;
    }

    CDuiString* pStrText = new CDuiString(strText);
    if (::PostMessage(GetPaintWindow(), WM_USER_UPDATE_UI_TEXT_ASYNC, (WPARAM)pCtrl, (LPARAM)pStrText))
    {
        return true;
    }
    delete pStrText;
...
bool CPaintManagerUI::UpdateProgressValueAsync(int nValue, CControlUI* pCtrl)
{
    if (!pCtrl || !::IsWindow(GetPaintWindow()))
    {
        return false;
    ...
    if (::PostMessage(GetPaintWindow(), WM_USER_UPDATE_UI_PROGRESS_VAL_ASYNC, (WPARAM)pCtrl, (LPARAM)nValue))
    {
        return true;
    }
    return false;
...
bool CPaintManagerUI::UpdateOptionStatusAsync(bool isSelected, CControlUI* pCtrl)
{
    if (!pCtrl || !::IsWindow(GetPaintWindow()))
    {
        return false;
    ...
    if (::PostMessage(GetPaintWindow(), WM_USER_UPDATE_UI_OPTION_STATUS_ASYNC, (WPARAM)pCtrl, isSelected ? TRUE : FALSE))
    {
        return true;
    }
    return false;
...
void CPaintManagerUI::AddDelayedDestroyedWnd(CWindowWnd* pWnd)
{
    if (::IsWindow(pWnd->GetHWND()))
    {
        pWnd->Close();
    }
    m_aDelayedDestroyedWnds.Add(pWnd);
    ::PostMessage(m_hWndPaint, WM_USER_DUILIB_DELAY_DESTROY_WNDS, 0, 0L);
}

void CPaintManagerUI::AddDelayedDestroyedThread(std::thread* pThread)
{
    m_aDelayedDestroyedThreads.Add(pThread);
    ::PostMessage(m_hWndPaint, WM_USER_DUILIB_DELAY_DESTROY_THREADS, 0, 0L);
}
